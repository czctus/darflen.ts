...
let debug = isBrowser ? null : await import(debugLit).then((m) => m.default).catch(() => null); // we do this because i don't want webworkers/browsers to bundle it..

const create = (namespace: string) => {
    if (isBrowser || !debug) return (..._args: unknown[]) => { };
